...
import type { AIActionBatch } from "./aiAction";
import type { TranscriptLookup } from "./transcriptLookup";
import { emptyTranscriptLookup } from "./transcriptLookup";

/** Browser/Electron-safe UUID (Web Crypto — available in renderers and Node 19+). */
...
function setSpeedForComposedRange(
	...
	for (const segment of segments) {
		...
		const fullyCovered = overlapStart <= segmentStart + 0.001 && overlapEnd >= segmentEnd - 0.001;
		if (fullyCovered) {
