...
export default function App() {
	const [windowType, setWindowType] = useState(() => {
		const param = new URLSearchParams(window.location.search).get("windowType");
		if (param) return param;
		// Tauri is single-window (no Electron multi-window) → boot straight into the editor.
		return isTauri() ? "editor" : "";
	});
	...
	useEffect(() => {
		const type =
			new URLSearchParams(window.location.search).get("windowType") || (isTauri() ? "editor" : "");
		if (type !== windowType) {
			setWindowType(type);
		}
