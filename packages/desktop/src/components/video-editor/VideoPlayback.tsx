...
import { decidePlayback, keptSpansFromTrim } from "@/lib/cutti/playback";
...
import { fromFileUrl, toMediaSrc } from "./projectPersistence";
import {
...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
	(
		...
		// cutti: enforce trim in the live preview by skipping cut gaps. No-op when
		// there are no trim regions, so openscreen's default playback is untouched.
		// timeupdate fires ~4x/s, so a skip can overshoot a cut by up to ~250ms —
		// good enough for v1; a frame-exact version needs the composed-clock rewrite.
		// biome-ignore lint/correctness/useExhaustiveDependencies: videoReady intentionally re-runs the effect so it attaches once the <video> element mounts
		useEffect(() => {
			const video = videoRef.current;
			if (!video) return;
			const onTimeUpdate = () => {
				if (!isPlayingRef.current || isSeekingRef.current || isScrubbingRef.current) return;
				const spans = keptSpansFromTrim(trimRegionsRef.current);
				if (spans.length === 0) return;
				const decision = decidePlayback(video.currentTime, spans);
				if (decision.kind === "seek") {
					if (Math.abs(video.currentTime - decision.toSec) > 0.03) {
						video.currentTime = decision.toSec;
					}
				} else if (decision.kind === "end" && !video.paused) {
					video.pause();
				}
			};
		...
		return (
			...
			>
						...
						(() => {
							...
							return (
								...
								>
									<video
										ref={webcamVideoRef}
										src={toMediaSrc(fromFileUrl(webcamVideoPath))}
					...
					{pixiReady && videoReady && (
						...
						>
							<div
								...
								className="absolute rounded-md border border-primary/80 bg-primary/20 shadow-[0_0_0_1px_rgba(52,178,123,0.35)]"
				...
				<video
					ref={videoRef}
					src={toMediaSrc(fromFileUrl(videoPath))}
					className="hidden"
					preload="auto"
					playsInline
				...
				{supplementalAudioPath && (
					<audio
						ref={supplementalAudioRef}
						src={toMediaSrc(fromFileUrl(supplementalAudioPath))}
						preload="auto"
					/>
				)}
			</div>
