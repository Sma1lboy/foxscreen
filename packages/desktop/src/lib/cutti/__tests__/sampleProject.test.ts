...
/**
 * End-to-end data-path check against a REAL cutti project.json
 * (examples/sample-project.json at the repo root), proving the engine's output
 * flows into openscreen's EditorState. The fixture lives at the repo root;
 * resolve it whether vitest runs with cwd = repo root or cwd = packages/desktop.
 */
const samplePath =
	["examples/sample-project.json", "../../examples/sample-project.json"].find((p) =>
		existsSync(p),
	) ?? "examples/sample-project.json";
const sample = JSON.parse(readFileSync(samplePath, "utf8")) as CuttiProject;
