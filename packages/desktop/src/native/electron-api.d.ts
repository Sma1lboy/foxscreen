// Global `window.electronAPI` typing for the renderer's native bridge. Inlined
// from the old electron/electron-env.d.ts during the Tauri migration (the runtime
// is provided by src/lib/tauri/electronApiShim.ts). Ambient on purpose — no
// top-level import/export, so `interface Window` stays a global augmentation.
...
interface Window {
	electronAPI: {
		invokeNativeBridge: <TData = unknown>(
			request: import("./contracts").NativeBridgeRequest,
		) => Promise<import("./contracts").NativeBridgeResponse<TData>>;
		getSources: (opts: {
			types?: string[];
		...
		setRecordingState: (
			recording: boolean,
			recordingId?: number,
			cursorCaptureMode?: import("../lib/recordingSession").CursorCaptureMode,
		) => Promise<void>;
		...
		startNativeWindowsRecording: (
			request: import("../lib/nativeWindowsRecording").NativeWindowsRecordingRequest,
		) => Promise<import("../lib/nativeWindowsRecording").NativeWindowsRecordingStartResult>;
		...
		startNativeMacRecording: (
			request: import("../lib/nativeMacRecording").NativeMacRecordingRequest,
		) => Promise<import("../lib/nativeMacRecording").NativeMacRecordingStartResult>;
		...
		attachNativeMacWebcamRecording: (payload: {
			...
			recordingId: number;
			webcam: import("../lib/recordingSession").RecordedVideoAssetInput;
			cursorCaptureMode?: import("../lib/recordingSession").CursorCaptureMode;
		}) => Promise<{
			success: boolean;
			path?: string;
